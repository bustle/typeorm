...
describe("basic-lazy-relations", () => {
    ...
    const resourceDir = appRootPath + "/test/functional/relations/lazy-relations/basic-lazy-relation/";
    ...
    it("should successfully load relations within a transaction", () => Promise.all(connections.filter((connection) => (new Set(["postgres"])).has(connection.options.type)).map(async connection => {
    ...
    it("should successfully load relations outside a transaction with entity generated within a transaction", () => Promise.all(connections.filter((connection) => (new Set(["postgres"])).has(connection.options.type)).map(async connection => {
